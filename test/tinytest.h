...
#define ASSERT_TRUE(expression) if (!tt_assert(__FILE__, __LINE__, (#expression), (expression) ? 1 : 0)) return
#define ASSERT_FALSE(expression) if (!tt_assert(__FILE__, __LINE__, (#expression), (expression) ? 0 : 1)) return

/* Convenient assertion methods */
/* TODO: Generate readable error messages for assert_equals or assert_str_equals */
#define ASSERT_EQUALS(expected, actual) ASSERT_TRUE((expected) == (actual))
#define ASSERT_STRING_EQUALS(expected, actual) ASSERT_TRUE(strcmp((expected),(actual)) == 0)
...
void tt_execute(const char* name, void (*test_function)())
{
  ...
  if (tt_current_test_failed) {
    printf("\nfailure: %s:%d: In test %s():\n   (%s)\n",
      tt_current_file, tt_current_line, name, tt_current_expression);
    tt_fails++;
  } else {
    tt_passes++;
  }
...
int tt_assert(const char* file, int line, const char* expression, int pass)
{
